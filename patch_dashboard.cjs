...
code = code.replace(/>لوحة القيادة</g, '>{t("dashboard.title")}<');
code = code.replace(/>نظرة عامة على أداء المبيعات والمؤشرات الرئيسية\.</g, '>{t("dashboard.subtitle")}<');
code = code.replace(/>إجمالي الإيرادات</g, '>{t("dashboard.revenue")}<');
code = code.replace(/>الصفقات النشطة</g, '>{t("dashboard.active_deals")}<');
code = code.replace(/>نسبة النجاح</g, '>{t("dashboard.success_rate")}<');
code = code.replace(/>مقارنة بالشهر السابق</g, '>{t("dashboard.vs_last_month")}<');
