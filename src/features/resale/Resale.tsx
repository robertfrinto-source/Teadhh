...
import { Plus, Search, Filter, Home, CheckCircle2, UserCircle, X } from "lucide-react";
...
export function Resale() {
  ...
  const addResaleUnit = useStore(state => state.addResaleUnit);
  ...
  return (
    <div className="flex flex-col w-full max-w-7xl mx-auto animate-fade-in relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="font-display-lg text-headline-lg sm:text-display-lg text-[#0F2D52] tracking-tight">{t("nav.resale")}</h1>
          <p className="font-body-md text-body-md text-on-surface-variant mt-1">إدارة وحدات إعادة البيع المعروضة في السوق.</p>
        </div>
          ...
          <table className="w-full text-right border-collapse">
            <thead>
              <tr className="bg-surface-container/30 border-b border-outline-variant/30">
                <th className="py-4 px-6 font-label-md text-label-md text-on-surface-variant whitespace-nowrap">{t("resale.type_unit")}</th>
                <th className="py-4 px-6 font-label-md text-label-md text-on-surface-variant whitespace-nowrap">{t("resale.price")}</th>
                <th className="py-4 px-6 font-label-md text-label-md text-on-surface-variant whitespace-nowrap">{t("rent.area")}</th>
                <th className="py-4 px-6 font-label-md text-label-md text-on-surface-variant whitespace-nowrap">{t("resale.owner")}</th>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-surface-container-lowest rounded-2xl p-6 w-full max-w-md shadow-lg animate-fade-in">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-headline-md text-on-surface">إضافة وحدة إعادة بيع</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-label-md mb-1 text-on-surface">{t("resale.type")}</label>
                <input 
                  type="text" 
                  value={newUnit.type} 
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <label className="block text-sm font-label-md mb-1 text-on-surface">{t("rent.area")}</label>
                  <input 
                    type="text" 
                    value={newUnit.area} 
              ...
              <div>
                <label className="block text-sm font-label-md mb-1 text-on-surface">السعر المطلوب</label>
                <input 
                  type="text" 
                  value={newUnit.price} 
              ...
              <div>
                <label className="block text-sm font-label-md mb-1 text-on-surface">اسم المالك</label>
                <input 
                  type="text" 
                  value={newUnit.owner} 
